...
36. **PEDIGREE + day shape + playbook + deep dive (2026-07-22, "full freedom, this is your project").** His sketch: our own algorithms for "how the stock is gonna do today / how it did", warnings about "some Chinese bullshit company", filters, and click-for-deep-research. Built as four connected pieces, all free-data, all fail-open:
   **(a) PEDIGREE (`src/pedigree.py`)** — "what am I actually holding": from the same EDGAR submissions JSON the dilution grade already fetches (plus the split history now pulled over ~13 months in `prepare()`'s single corporate-actions call): CN-LINKED (incorporation or business address in mainland China/HK — EDGAR codes F4/K3), OFFSHORE SHELL (Cayman E9 / BVI D8 / Marshall 1T / Bermuda C5), SERIAL DILUTER (≥3 offering/registration filings in 12mo), REVERSE SPLITS (count ≥1 in ~13mo), MICRO FLOAT (<5M), FRESH REGISTRANT (first visible filing <18mo, only claimed when the history isn't API-truncated). Grade: CLEAN RECORD / n FLAGS / PUMP ANATOMY n/6. **Design intent, do not drift from it: these names are what he trades — pedigree NEVER gates or hides anything; it labels the vehicle so he rides the wave without marrying it.** Cached 7 days; watchlist names only (budget); intel pass carries it in `latest_intel.json["ped"]`.
   **(b) Day shape (`watch.day_shape`)** — "how it did", factually: GAP & GO / GAP & FADE / GAP & CHOP / RUNNER / FADED RUNNER / MIDDAY POP / BLEEDER / DRIFT, plus a timeline with times ("open 1.40 (gap +40%) · high 2.08 @ 10:10 · low … · now … (-38% off high)"). No prediction lives in the classifier.
   **(c) Playbook line (`hub._playbook`)** — the only forward-looking sentence, fusing shape × pedigree flag count, ALWAYS labeled "hist" (the item-26 rule): pump-fade anatomy → "scalp the bounces, don't marry it"; running on ≥3 flags → "momentum is real until the offering prints"; clean momentum → "vwap dips historically get bought (hist, not a call)".
   **(d) Deep dive (`hub._dossier`)** — native `<details>` on every MY NAMES card, pre-rendered server-side and shipped inside `watch.json` rows so the client injects the exact same HTML: grade + every flag with its factual basis, jurisdiction, reverse splits, today's timeline, last 5 filings, and a direct link to the company's full SEC filing history (the only external link on the page; navigation, not a data sink). Open state survives the 45s re-render.
   **Proven:** 62 fixture checks incl. the full archetype (Cayman-inc/China-biz serial-diluting reverse-splitting 2M-float fresh registrant → PUMP ANATOMY with every flag named), clean-US-company → CLEAN RECORD, single-shelf ≠ serial, gap-fade classification with timeline, playbook/dossier rendering, demo end-to-end. **Unproven:** EDGAR jurisdiction codes against a live pull for his actual names (the F4/K3/E9/D8 mapping is per EDGAR's documented code list); shape thresholds against his eye — if he says a label is wrong, tune `day_shape`'s constants and add the fixture.

37. **WHOSE money is it — buy/sell side + book pressure + the free second pipe (2026-07-22).** His correction to item 36: pedigree is "a tiny thing"; for a 10-minute hold he needs "if there's money coming in there on my side buying, or if more selling orders are about to come in" — and he wants it free, not via the paid SIP add-on.
   **(a) Side classification, free, from the stream we already have.** The live lane now subscribes QUOTES alongside trades on the same Alpaca socket; each print is classified against the prevailing NBBO (at/above ask = buyer paying up, at/below bid = seller dumping; tick-rule fallback when no quote). The strip shows the split — "▲$4.2k buy / ▼$1.1k sell (79% buyers)" — and the headline is side-aware: FLOW splits into **BUYERS IN** / **SELLERS HITTING** at a 65/35 dollar share; unclassified prints never fake a side (bshare stays null → neutral MONEY IN).
   **(b) "Sell orders about to come in" — the honest free proxy.** True L2 depth is not free anywhere; the top of the book is: when NBBO ask size stacks ≥2.5× bid size, the strip appends "book: ask 3.1× stacked — sellers queuing" (and the mirror for bids). Ratio only — unit-independent, so lot-size conventions can't produce wrong dollar math. Quotes older than 15s are ignored.
   **(c) Finnhub second pipe.** His FINNHUB_KEY (⚙ panel field, same value as the repo secret) opens finnhub's own free live US trade websocket — a different source, so no conflict with Alpaca's one-connection limit. Per name, the strip feeds from whichever pipe printed more in the last 60s (finnhub needs 1.5× density to win; line says "via finnhub"); finnhub prints carry exchange timestamps, and if the median lag exceeds 5s the pipe is dropped for the session and logged — **a delayed feed presented as live would be a lie; keep this guard.** Finnhub-side prints classify by tick rule only (no free NBBO there).
   **Proven:** node fixtures — all-at-ask flow → BUYERS IN, all-at-bid → SELLERS HITTING, side split in the label, ask/bid stacking lines, stale-quote ignored, legacy no-side buffers stay neutral; sink audit now allows exactly github + alpaca + finnhub and asserts the finnhub URL carries only the finnhub key. **Unproven:** Alpaca quote-message field shapes at scale from a phone, finnhub free-tier lag on his names (the 5s guard is the answer either way), and the 65/35 side thresholds against his read of the tape.

38. **Live chart + strict-minimal overhaul (2026-07-22, closing rounds).** (a) Every MY NAMES card carries a canvas: last 2 min of tape — price polyline glowing in the live-state color, last-print dot, buy/sell dollars as green/red 3s bars — repainted every second from the stream buffer (`drawSpark`). (b) Then his design verdict: "way too much slop… one stock reads cooling and heating and surging… minimalistic, only what we need, nothing static." Cards are now EXACTLY: ticker · live word · price / chart / live line — everything else (all chips incl. mood/state/pedigree/shape, story, EDGE verdict, playbook, dossier) folded behind one `<details class=mre>` "more"; the section explainer removed. **Rule going forward: one label owns a card — the per-second read, engine mood only as its fallback. Never stack verdict words in view.** Open state survives re-renders; painter ids (`wc_/lvh_/lv_/sp_`) are the contract between both renderers.

39. **"It just says DEAD" + the next overhaul — HIS REDIRECT (2026-07-22, end of day, read this first next session).** He rejected the details-fold as "just hiding," and made three points that define the next round: (a) a card said DEAD while his broker showed orders flowing — the free IEX slice sees minutes between prints on thin names and we presented sample-absence as a verdict. FIXED this session: `mood_candidate` now returns **THIN TAPE** (never DEAD/LEAVING) under $3k/15m of sample, with the story saying "feed sample too thin to judge — broker tape may show more." Extend the same guard to the hub live strip's DRY/SILENT words and show per-name prints/min so liveness is visible. (b) He can't tell what's live vs stale: STAMP EVERY READ — the story/mood are the 45s engine lane (say "engine · 13:42"), the word/chart/line are per-second (say "● live"). (c) NEWS must be on the card face, not folded: persist newswire hits per watchlist symbol to a day file in the fast lane, render headline + age + 🔴 chip top-level. Latencies as of today, tell him when asked: chart/word/live-line = per-second (phone stream); story/mood/state = one engine tick, 45–160s, plus ~60–90s Pages deploy (so 2–3 min worst case); EDGAR/pedigree = cached hours-days. His closing instruction verbatim-adjacent: "I think what I'm telling you is wrong and you follow me and you're building wrong things… don't make it simple. Make some algorithms." Meaning: stop literal-following; build discriminating signal — the thin-feed guard is the model. Candidates he'd value: cross-name rotation detection (money leaving X entering Y among HIS names), open-drive vs fade discrimination in the first 15 min, and halt-resume behavior reads. The fold-under-"more" layout is NOT approved as final — rethink the card as a chart-first instrument, not text with an expander.

40. **THE ARCHITECTURE VERDICT (2026-07-22, his "why the fuck do we need it if it updates every 3 minutes" — he is right; NEXT SESSION EXECUTES THIS).** Step-back: the GitHub lane (engine→commit→Pages, 45s-3min) is legacy of the discovery mission. His mission is now HIS 4-8 names at second scale. **Rebuild the hub as a self-contained client-side terminal:** everything he sees mid-trade computed on-device — day%/vwap/off-high from Alpaca REST minute bars pulled client-side (data.alpaca.markets; verify browser CORS first — believed yes) then maintained live from the stream; NEWS per symbol client-side from Finnhub REST company-news with his existing key (finnhub supports browser CORS) polled ~45s; live halt inference from the stream itself (prints stop + quote frozen). The GitHub lane keeps ONLY: watchlist sync, slow research (EDGAR/pedigree/halt history), nightly scan, discovery board below — all visually dated as background context, never presented as current. Kill the mood/story as card-level reads or stamp them as archive. **Databento asked about, answered honestly: professional usage-priced feed with a raw binary live protocol — needs a server between him and it; doesn't fit phone+static-page without infrastructure and real cost. The browser-ready ladder stays: free (Alpaca IEX+Finnhub merged) → ~$79-99/mo (Polygon or Alpaca SIP full tape, drop-in) → server-based feeds (Databento/dxFeed) only if he ever wants a hosted backend.** Item 39's specifics (liveness stamps, news chip, thin-guard on strip, plain wording, rotation-between-his-names algorithm) fold INTO this rebuild rather than being separate patches.

41. **THE CORE READ IS WRONG — replace per-second verdicts with CUMULATIVE DELTA (2026-07-22 close, his final and sharpest bug report: "it says dead and then suddenly alive, money coming in… I'm literally looking at green bar red bar every second, what the fuck is the point").** Diagnosis he forced: on the IEX slice his names print every 30-60s; ANY per-second verdict on an event-sparse sample can only whipsaw between nothing and something — the flips and the per-print bars are noise rendered, not signal. **The rebuild's card core (fold into item 40's terminal): CVD — cumulative buys-minus-sells in dollars (NBBO-classified, the item-37 classifier) since session start, drawn as a line under/next to price.** Rising = money coming in on his side; flattening = absorption ending; rolling over = distribution. It accumulates instead of judging each second, so it is flicker-proof by construction and honest on sparse tape. ONE word per card derived from CVD slope over the last 3-5 minutes (density-adaptive: dense tape may use shorter windows; sparse tape must not), replacing the FLOW/DRY/DEAD word machine entirely on the hub. Kill the per-3s green/red bars; keep the price line; add the delta line. The engine mood chip never appears as a live read again (archive context only, stamped). This is the "make some algorithms" he meant.

42. **HE ASKED FOR PREDICTION — REVIVE NEXT-WINDOW WITH THE SCOREBOARD (2026-07-22 final order: "it needs to compute… not just tell me what already happened… look at history… build this for me").** This is the item-26 forecast, now explicitly demanded by the user. The rule stands: ship it ONLY with live receipts. Rebuild spec (third pillar of items 40+41's terminal): (a) per name, a NEXT read (next 60s / next 5m flow: BUILDING / DRYING, direction lean from CVD slope + burst state + book imbalance); (b) EVERY call auto-graded on-device against the realized window, sampled ≤1/symbol/20s (item 26's anti-inflation rule); (c) the card shows the running hit-rate on HIS names with the advisory: ≥60% "calls are carrying", ≤40% "your tape fades the call — invert it", else "coin flip — ignore"; (d) a history layer: match the current session's shape+time against the gapper-stats priors AND his accumulating journal ("days that looked like this at 10:30 → what happened by 2pm"), always labeled hist with n. The scoreboard IS the feature — never ship the number without it. Combined with 40 (client-side terminal) + 41 (CVD core), this is the project he is asking for; build these three together, not piecemeal.
